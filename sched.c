...
need_resched_back:
	...
	repeat_schedule:
		...
		if(phase2switchflag==1){
			...
			list_for_each(tmp, &runqueue_head) {
				...
				if (can_schedule(p, this_cpu)) {
					...
					if (p->phase2pass < minpass) {   
						minpass = p->phase2pass; 
						next = p;
		...
		next->phase2pass = next->phase2pass + next->stride;
